...
import { Expand, Music, Pause, Play, RotateCcw, VolumeX } from "lucide-react";
...
function PreviewSidebar({
  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed bottom-0 right-0 p-2 md:p-3 lg:p-4 z-10">
        <button
          onClick={() => toggleFullscreen?.()}
          className="pointer-events-auto md:size-12 size-10 lg:size-15 rounded-full bg-blue-500 hover:bg-blue-600 flex items-center justify-center transition-colors shadow-lg touch-none"
          aria-label="Enter fullscreen"
          title="Enter fullscreen"
        >
          <Expand className="lg:size-8 md:size-8 size-7 text-white" />
        </button>
      </div>
    </>
  );
